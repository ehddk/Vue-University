			...
			<div class="login_content">
				<span class="login_input">
					...
					<input
						type="password"
						placeholder="비밀번호"
						style="display: block; margin-top: 10px; padding: 10px; height: 30px; width: 300px"
						v-model="user.password"
					/>

					<button class="log_btn" @click="login" style="margin-top: 10px; padding: 20px; height: 30px; width: 324px"><span>로그인</span></button>
					<button class="join_btn" @click="moveJoin" style="display: block; margin-top: 10px; padding: 20px; height: 30px; width: 324px">회원가입</button>
				</span>
			</div>
			<div class="save">
				<div class="id_save">
...
export default {
	...
	methods: {
		login: function () {
			this.$axios.post('/api/user/login', this.user).then((result) => {
